...
export async function getLatestUpdateBundlePathForRuntimeVersionAsync(
  ...
  const updatesDirectoryForRuntimeVersion = `updates/${projectName}/${runtimeVersion}`;
...
export async function getAssetMetadataAsync(arg: GetAssetMetadataArg) {
  ...
  return {
    ...
    contentType,
    url: `${process.env.HOSTNAME}/api/assets?asset=${assetFilePath}&runtimeVersion=${arg.runtimeVersion}&platform=${arg.platform}&project=${arg.projectName}`,
  };
}
